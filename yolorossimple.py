...
image_pub = rospy.Publisher('drone_image', Image, queue_size=1)
imagecompress_pub = rospy.Publisher('drone_image/compressed', CompressedImage, queue_size=10)
publisher = rospy.Publisher('position', Float32MultiArray, queue_size=10)
bridge = CvBridge()
...
class Demo:
    ...
    def loop(self):
        ...
        if self._conf.useCamera:
            if self._nnManager is not None:
                ...
                result=self._nnManager.draw(self._pv, self._nnData)
                if result is not None:
                    print(result)
                    if result[7]=="person":
                        position_msg.data = [result[0],result[1],result[2],result[3],result[4],result[5],result[6]]
    ...
    def _showFramesCallback(self, frame, name):
        ...
        if name=='color'and frame is not None:
            ros_image = bridge.cv2_to_imgmsg(frame, encoding='bgr8')
            compressed_image_msg = bridge.cv2_to_compressed_imgmsg(frame, "jpeg")
            publisher.publish(position_msg)
            image_pub.publish(ros_image)
            imagecompress_pub.publish(compressed_image_msg)
